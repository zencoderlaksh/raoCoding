  ...
  useEffect(() => {
    ...
    const updatePointer = event => {
      ...
      if (fixed) {
        ...
        x = clamp(event.clientX, 0, w);
        y = clamp(h - event.clientY, 0, h);
      } else {
  ...
  return (
    <>
      <div
        ...
        className={
          fixed
            ? `pointer-events-none fixed inset-0 z-40 overflow-hidden ${className}`.trim()
            : `relative h-full w-full overflow-hidden${className ? ` ${className}` : ''}`
        }
        style={fixed ? { width: '100vw', height: '100vh', ...style } : style}
        {...rest}>
        <canvas
          ref={canvasRef}
      ...
      {children && (fixed ? children : <div className="relative z-[1] h-full w-full">{children}</div>)}
    </>
  );
};
